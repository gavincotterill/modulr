...
test_that("plot_transmissions works", {
  ...
  g <- igraph::graph_from_edgelist(gc[-1,1:2] %>% as.matrix())
  igraph::V(g)$membership <- stringr::str_extract(igraph::V(g)$name, "\\d{1,}(?=_)") %>% as.numeric()
  expect_silent(plot_transmissions(g))
})
