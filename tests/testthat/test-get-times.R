...
test_that("get_times works", {
  ...
  s2 <- simulate_schedule(n_animals = na, n_groups = 3, n_splits = NA, time_to_leave = 5, time_to_return = 2, travel_time = c(0.001, 0.002), sampling_duration = 14, simulator = "group-think")

  ids2 <- names(s2)

  out2 <- get_times(schedule = s2[[1]], id = ids2[[1]], simulator = "group-think")

  for (i in 2:length(ids2)) {
    out2[i, ] <- get_times(s2[[i]], ids2[[i]], simulator = "group-think")
  }
