#' sglt_bind sub group list transformation and binding
#'
#' @keywords internal
sglt_bind <- function(a, b){
  t0 <- a["locations"][[1]]
  t_home <- t0 %>% dplyr::filter(at_home == "home")
  t_away <- t0 %>% dplyr::filter(at_home == "away")
  ...
  sub_group_list <- lapply(sub_group_list, function(x){dplyr::arrange(x, cumulative_time) %>% tidyr::fill(sub_id, .direction = "up")})
