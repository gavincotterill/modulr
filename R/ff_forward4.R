...
#' @keywords internal
#'
ff_forward4 <- function(t2, curr_vec, mbrs_list, i, time_to_leave, time_to_return){
  ...
  # if( (any(c("fission", "fission-fusion") %in% next_actions) & length(curr_vec) > 1) | (next_actions %in% c("same-trigger")) ){
  # if(any(c("fission", "fission-fusion", "same-trigger") %in% next_actions) ){ # AND they changed states
  ...
  if(any(c("fission", "fission-fusion", "same-trigger") %in% next_actions)){
