...
simulate_non_independence2 <- function(
  ...
  for(i in 1:nrow(t2)){
    ...
    t2$action[i] <- delta_grp2(t2, "vector", value, i)
  }
  ...
  for(i in 1:nrow(t2)){
    ...
    if(t2$action[i] %in% c(NA)){
      ...
      t2 <- ff_forward4(t2, curr_vec, mbrs_list, i, time_to_leave, time_to_return)
      ...
      mbrs_list <- as.list(stringr::str_split(t2$members[i], "/")[[1]]) %>% `names<-`(curr_vec)
      t2 <- ff_forward4(t2, curr_vec, mbrs_list, i, time_to_leave, time_to_return)
    } else if(t2$action[i] %in% c("fission", "fission-fusion", "same-trigger")){
