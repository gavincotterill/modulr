...
#' @param simulator the simulator used to generate the schedule. One of: "independent", "non-independent", "group-think".
#' @export
get_times <- function(schedule, id, simulator, option = NA){
  if(simulator %in% c("independent", "non-independent")){
    ...
    p <- schedule %>% dplyr::mutate(time = .data$end - .data$start)
    p3 <- p %>% dplyr::filter(.data$state == as.numeric(grp))
    p4 <- p %>% dplyr::filter(.data$state != as.numeric(grp))
