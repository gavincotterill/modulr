...
A closer judge of it's performance -- albeit still imperfect -- is the amount of time individuals spent 'attached' to the core of their home group, which floats around between patches. We'll just overwrite the previous objects, but this time we give `get_times()` the correct simulator name:
...
![](./simulator-comparison-ggplot-proportion-time-1.png)
...
![](./simulator-comparison-plot-graphs-1.png)
...
![](./simulator-comparison-edge-weights-1.png)
...
We could have predicted that the non-independent simulator would produce a higher modularity value than the others based on everything we've said up to this point. In general they should be within about 10% of each other.

Now let's do a longer run with a larger network. We'll create a network of 150 animals distributed across 13 groups (and patches). That should give us some bigger differences in group sizes. We'll double the sampling time to 30 days and leave all else the same.
...
![](./simulator-comparison-ggplot-proportion-time2-1.png)

Now 'indepedent' and 'non-independent' have converged to similar distributions. 'Group-think' still looks chaotic, but better. Because of what's happening behind this scenes this isn't a perfect comparison for this simulator.
...
![](./simulator-comparison-plot-graphs2-1.png)
...
![](./simulator-comparison-edge-weights2-1.png)
...
The modularity scores haven't changed much from before. Independent increased from 0.38 to 0.44, the non-independent score is basically identical, and group-think has decreased very slightly -- although you might not guess it by looking at the graphs.
